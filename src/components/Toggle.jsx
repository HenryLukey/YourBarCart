...
const Toggle = ({ initialState, value, onToggle }) => {
    ...
    return (
        ...
        <div className={`m-2 border hover-rounded-lg text-left px-1 p-1 border-darkColour select-none ${state ? "bg-primary hover:bg-primaryVariant"  : "bg-lightColour hover:bg-lightVariant dark:bg-darkModeMain dark:hover:bg-darkModeVariant"}`} onClick={toggle}>{value}</div>
    );
}
